...
function RegisterListeners() {
  document
    .getElementById('mobius1-x1')
    .addEventListener('input', UpdateImgs)
  document
    .getElementById('mobius1-x2')
    .addEventListener('input', UpdateImgs)
  document
    .getElementById('mobius2-x1')
    .addEventListener('input', UpdateImgs)
  document
    .getElementById('mobius2-x2')
    .addEventListener('input', UpdateImgs)
}
...
function UpdateImgs() {
  point1 = [
    parseFloat(
      document.getElementById('mobius1-x1').valueOf()['value'],
    ),
    parseFloat(
  ...
  point2 = [
    parseFloat(
      document.getElementById('mobius2-x1').valueOf()['value'],
    ),
    parseFloat(
...
function UpdateMobiusLine() {
  ...
  for (let i = 0; i < 1; i += 0.01) {
    ...
    mobius_line_ctx.fillRect(
      Math.round(pos[0] * SCALE),
      Math.round(pos[1] * -SCALE),
      1,
      1,
    )
  }
  ...
  for (let i = -50; i < 50; i++) {
    for (let j = -50; j < 50; j++) {
      if (norm([i / SCALE, j / SCALE]) <= 1.01) {
        let new_coord = c_mult(
          SCALE,
          MobiusGyration(point1, point2, [i / SCALE, j / SCALE]),
        )
        new_coord = [Math.round(new_coord[0]), Math.round(new_coord[1])]
        let pixel = mobius_gyr_def_ctx.getImageData(
          i + 250,
          j + 250,
          1,
          1,
        ).data
        mobius_gyr_ctx.fillStyle =
          'rgba(' +
          pixel[0] +
          ',' +
          pixel[1] +
